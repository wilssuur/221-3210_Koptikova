    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                DbgPrint("PassThrough write\n");

                if (Data->Iopb->Parameters.Write.WriteBuffer) {
                    uint8_t buffer[1024];
                    memcpy(buffer, Data->Iopb->Parameters.Write.WriteBuffer, 1024);
                    struct AES_ctx ctx;
                    uint8_t key[32] = "tqaaAMrijpJYXPSArhoSjANTpvojqCbl";
                    uint8_t iv[16] = "FeilsFUZUdyQHghi";
                    AES_init_ctx_iv(&ctx, key, iv);
                    AES_CBC_encrypt_buffer(&ctx, &buffer, 1024);
                    memcpy(Data->Iopb->Parameters.Write.WriteBuffer, buffer, 1024);
                }
    ...
    if (!NT_SUCCESS(status))
    {
        ...
        if (
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_READ) {
                ...
                if (Data->Iopb->Parameters.Read.Length > 1) {
                    uint8_t buffer[1024];
                    memcpy(buffer, Data->Iopb->Parameters.Read.ReadBuffer, 1024);
                    struct AES_ctx ctx;
                    uint8_t key[32] = "tqaaAMrijpJYXPSArhoSjANTpvojqCbl";
                    uint8_t iv[16] = "FeilsFUZUdyQHghi";
                    AES_init_ctx_iv(&ctx, key, iv);
                    AES_CBC_decrypt_buffer(&ctx, &buffer, 1024);
                    memcpy(Data->Iopb->Parameters.Read.ReadBuffer, buffer, 1024);
                }
